...
public sealed class RightClickHook : AbstractHook
{
    ...
    internal static int Usages { get; set; }

    /// <inheritdoc />
    public override void Configure(Module mod)
    {
        mod.GlobalLoad.AddDelegate(ctx =>
        {
            ctx.RunUnsafe(
                $"scoreboard objectives add right_click_{Usages} minecraft.used:minecraft.carrot_on_a_stick");
        ...
        {
            ctx.RunUnsafe(
                $"execute as @a[scores={{right_click_{Usages}=1..}}] at @s run function {mod.Namespace}:hooks/right_click/internal_{Usages}");
